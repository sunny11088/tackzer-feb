...
const Header = () => {
    ...
    const handleDownloadBrochure = () => {
        const brochureUrl = "/trackzer-company-profile.pdf"; // Ensure the file is in the 'public' folder
        const link = document.createElement("a");
        link.href = brochureUrl;
        link.setAttribute("download", "Trackzer-Company-Profile.pdf");
                        ...
                        <div className="col-md-6">
                            <div className="quick-contact">
                                <div className="quick-contact-right">
                                    <ul>
                                        <li>
                                            <Link to="https://www.facebook.com/TrackzerSwitchgears" target="_blank" rel="noopener noreferrer">
                                                <span className="connect-icon">
                                                    <img src={facebookIcon} alt="Facebook" className="social-icon"/>
                                                </span>
                                            </Link>
            ...
            <div className="container custom-container">
                <nav className="navbar-my">
                    <div className="navbar-container">
                        ...
                        <ul className={`navul ${isMenuOpen ? 'open' : ''}`}>
                            <li className="nav-item">
                                <NavLink to="/" end onClick={closeAllMenus} className="nav-link">Home</NavLink>
                            </li>
                            <li className="nav-item">
                                <NavLink to="/aboutus" onClick={closeAllMenus} className="nav-link">About Us</NavLink>
                            </li>
                            <li className={`nav-item relative ${openSubmenuIndex === 0 ? 'hover' : ''}`}
                                onMouseEnter={() => setOpenSubmenuIndex(0)}
                                onMouseLeave={() => setOpenSubmenuIndex(null)}>
                                <NavLink to="/products" onClick={closeAllMenus} className="nav-link">Products</NavLink>
                                <span className="caret" onClick={(e) => toggleSubmenu(0, e)}>
                                    <img src={caretsign} alt="Toggle submenu" width="18" height="18"/>
                                </span>
                                <ul className={`submenu ${openSubmenuIndex === 0 ? 'open' : ''}`}>
                            ...
                            <li className="nav-item">
                                <button onClick={handleDownloadBrochure} className="nav-link"
                                        aria-label="Download Brochure">
                                    Download Brochure
                                </button>
                            </li>

                           {/* <a href="/../../src/assets/trackzer-company-profile.pdf" className="nav-link"
                               onClick={closeAllMenus} aria-label="Download Brochure" download>
                                Download Brochure
                            </a>
*/}

                            <li className="nav-item">
                                <NavLink to="/contactus" onClick={closeAllMenus} className="nav-link">Contact
                                    Us</NavLink>
                            </li>
                        </ul>
                        <button className="menu-toggle" onClick={toggleMenu} aria-label="Toggle menu">☰</button>
